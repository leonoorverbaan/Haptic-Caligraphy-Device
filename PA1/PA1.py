...
gaussian = z * np.exp(-((x - mx) ** 2 / (2 * sx ** 2) + (y - my) ** 2 / (2 * sy ** 2)))
gradient_map = np.array(np.gradient(gaussian))
...
while run:
    ...
    #if gradToggle:
    #    grad_dx = gradient_map[0][xh[0]][xh[1]]
    #    grad_dy = gradient_map[1][xh[0]][xh[1]]
    #    k_shape = k
    #    f_shape = k_shape * np.array([grad_dx, grad_dy])

        # Update the force fe according to the response of the virtual environment
    #    fe = fe + f_damper + f_shape
    ...
    if haptic.colliderect(wall):
        ...
        if proxy_old[0] <= 300:
            proxy = pygame.Rect(300 - proxy_x / 2, xh[1], 0, 0).inflate(proxy_x, 48)
            fe[0] += kc * (xh[0] - proxy.centerx)
            #print(fe)
        else:
            proxy = pygame.Rect(xh[0], 200 - proxy_y / 2, 0, 0).inflate(48, proxy_y)
            fe[1] += kc * (xh[1] - proxy.centery)
            #print(fe)
